...
func (m *OpenConfigTelemetry) Start(acc telegraf.Accumulator) error {
	...
	for _, server := range m.Server {
		...
		if m.CertFile != "" {
			...
			m.grpcClientConn, err = grpc.Dial(server, grpc.WithTransportCredentials(creds))
			log.Printf("I! Opened a secure channel to %s on port %s", grpc_server, grpc_port)
		} else {
			m.grpcClientConn, err = grpc.Dial(server, grpc.WithInsecure())
			log.Printf("I! Opened an insecure channel to %s on port %s", grpc_server, grpc_port)
		}
		if err != nil {
		...
		if m.Username != "" && m.Password != "" && m.ClientId != "" {
			...
			log.Printf("I! Successfully authenticated using username, password for device %s", grpc_server)
		} else {
			log.Printf("I! Using no authentication for device %s", grpc_server)
		}
		...
		for _, sensor := range m.Sensors {		
			go func(sensor string, acc telegraf.Accumulator) {
				...
				log.Printf("Subscribing to %s to device %s on port %s", sensorName, grpc_server, grpc_port)
				for {
					var r *telemetry.OpenConfigData
					var errRecv error
					log.Printf("Sending subscription request for %s to device %s on port %s", sensorName, grpc_server, grpc_port)
					stream, err := c.TelemetrySubscribe(context.Background(),
						&telemetry.SubscriptionRequest{PathList: pathlist})
					log.Printf("Subscription Done for %s to device %s on port %s", sensorName, grpc_server, grpc_port)
					if err != nil {
